...
/*moderate the whole process of KMeans*/
public class KMeansDriver {
	...
	public void clusterCenterJob() throws IOException, InterruptedException, ClassNotFoundException{
		for(int i = 0;i < iterationNum; i++){
			...
			clusterCenterJob.setInputFormatClass(TextInputFormat.class); 
			clusterCenterJob.setOutputFormatClass(TextOutputFormat.class);
	...
	public void KMeansClusterJod() throws IOException, InterruptedException, ClassNotFoundException{
		for(int i=0;i<iterationNum;i++) {
			...
			kMeansClusterJob.setJobName("KMeansClusterJob"+i);
			kMeansClusterJob.setJarByClass(KMeansCluster.class);
		
			kMeansClusterJob.getConfiguration().set("clusterPath", outputPath + "/cluster-" + i +"/");
			...
			kMeansClusterJob.setInputFormatClass(TextInputFormat.class); 
			kMeansClusterJob.setOutputFormatClass(TextOutputFormat.class);
