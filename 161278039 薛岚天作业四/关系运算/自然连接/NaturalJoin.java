...
public class NaturalJoin {
	public static class NaturalJoinMap extends Mapper<Object,Text, Text, Text>{
		private int col;
		@Override
		...
		IOException, InterruptedException{
			...
			//decide which file we are dealing with
			if(filename.contains("a")) {
				RelationA record=new RelationA(value.toString());	
				context.write(new Text(record.getCol(col)), 
						new Text(filename.toString() + " " + record.getValueExcept(col)));
			}
			if(filename.contains("b")) {
				RelationB record =new RelationB(value.toString());
